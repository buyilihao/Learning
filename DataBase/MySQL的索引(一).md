...
![image_1ca22tt9f15kg9h114cq1dl81mlbm.png-89.6kB](image\索引\image_1ca22tt9f15kg9h114cq1dl81mlbm.png)
...
![image_1caac8jr7hhcld017gd1lch1n1m33.png-97.1kB](image\索引\image_1caac8jr7hhcld017gd1lch1n1m33.png)
...
![image_1caacokob6ne1nv41meda0s7vk3g.png-68.1kB](image\索引\image_1caacokob6ne1nv41meda0s7vk3g.png)

把一些记录放到页里边的示意图就是：

![image_1caadhc4g1pb7hk81fcd4vt1u6r3t.png-79.8kB](image\索引\image_1caadhc4g1pb7hk81fcd4vt1u6r3t.png)
...
- **下一个数据页的主键值必须大于上一个页中的主键值。**
  ...
  ![image_1caaf26411d51bq7jtrvesr04a.png-29.5kB](image\索引\image_1caaf26411d51bq7jtrvesr04a.png)
  ...
  ![image_1caafbcj1qpo1ad2j8q1ci4136s4n.png-44.5kB](image\索引\image_1caafbcj1qpo1ad2j8q1ci4136s4n.png)
  ...
  ![image_1caafkq3h1akv1mde14h2kjul6554.png-96.9kB](image\索引\image_1caafkq3h1akv1mde14h2kjul6554.png)
...
- 给所有的页建立一个目录项。
  ...
  ![image_1cab9u9midn61fgq1mi58j0gadm.png-65.7kB](image\索引\image_1cab9u9midn61fgq1mi58j0gadm.png)
...
- ![image_1caba0afo11fa1cli1nu070m16bg1j.png-119.1kB](image\索引\image_1caba0afo11fa1cli1nu070m16bg1j.png)

- 以`页28`为例，它对应`目录项2`，这个目录项中包含着该页的页号`28`以及该页中用户记录的最小主键值`5`。我们只需要把几个目录项在物理存储器上连续存储，比如把他们放到一个数组里，就可以实现根据主键值快速查找某条记录的功能了。比如说我们想找主键值为`20`的记录，具体查找过程分两步：
...
![image_1caahuomf15m11e5k19v1bf21inq9.png-145.9kB](image\索引\image_1caahuomf15m11e5k19v1bf21inq9.png)
...
![image_1cacabsrh17a5133q1otf725gi92q.png-135.7kB](image\索引\image_1cacabsrh17a5133q1otf725gi92q.png)
...
![image_1cacafpso19vpkik1j5rtrd17cm3a.png-158.1kB](image\索引\image_1cacafpso19vpkik1j5rtrd17cm3a.png)
...
![image_1ca80gps314u9121u1rdp9r7md8cm.png-55.6kB](image\索引\image_1ca80gps314u9121u1rdp9r7md8cm.png)
...
![image_1cactc8jg14j91likvmd1h8cn3o4h.png-161.6kB](image\索引\image_1cactc8jg14j91likvmd1h8cn3o4h.png)
...
为什么我们还需要一次`回表`操作呢？直接把完整的用户记录放到`叶子节点`不就好了么？你说的对，如果把完整的用户记录放到`叶子节点`是可以不用`回表`，但是太占地方了呀～相当于每建立一棵`B+`树都需要把所有的用户记录再都拷贝一遍，这就有点太浪费存储空间了。因为这种按照`非主键列`建立的`B+`树需要一次`回表`操作才可以定位到完整的用户记录，所以这种`B+`树也被称为`二级索引`（英文名`secondary index`），或者`辅助索引`。由于我们使用的是`c2`列的大小作为`B+`树的排序规则，所以我们也称这个`B+`树为为`c2`列建立的索引。
...
![image_1cacvu3hmlr9m501r6b1809uo35o.png-165.1kB](image\索引\image_1cacvu3hmlr9m501r6b1809uo35o.png)
...
- 将表中的记录按照插入时间顺序的存储在一块存储空间上，我们可以通过行号而快速访问到一条记录（因为`index_demo`表的记录是定长的，所以可以使用行号来进行快速访问，对于变长的记录MyISAM有不同的处理方案，我们这里就不介绍了），如图所示：
  ...
  ![image_1cad0lhac1qq31jcf1t4oa2a5c5p.png-41.8kB](image\索引\image_1cad0lhac1qq31jcf1t4oa2a5c5p.png)
