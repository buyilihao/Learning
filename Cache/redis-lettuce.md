...
[![img](assets/1412331-20190928093229322-1599957688.png)

既然能被`Spring`生态所认可，`Lettuce`想必有过人之处。编写本文时所使用的版本为`Lettuce 5.1.8.RELEASE`，`SpringBoot 2.1.8.RELEASE`，`JDK [8,11]`。
...
- `RedisCommands`：`Redis`命令`API`接口，**基本上覆盖了Redis发行版本的所有命令**，提供了同步（`sync`）、异步（`async`）、反应式（`reative`）的调用方式，对于使用者而言，会经常跟`RedisCommands`系列接口打交道。
