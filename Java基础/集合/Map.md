...
**特点**：用于保存具有映射关系的数据，具有两组值：一组用于保存Map中的key；另一组用于保存Map中的value，形成key-value的存储形式。
...
+ **HashMap**
  ...
  最常用的Map，**JDK8.0之前数组-链表实现，之后数组-链表-红黑树实现**。它根据键的HashCode 值存储数据，根据键可以直接获取它的值，具有很快的访问速度。HashMap最多只允许一条记录的键为Null(多条会覆盖);允许多条记录的值为 Null。非同步的。

+ **LinkedHashMap**

  链表-数组实现。保存了记录的插入顺序，在用Iterator遍历LinkedHashMap时，先得到的记录肯定是先插入的.在遍历的时候会比HashMap慢。key和value均允许为空，非同步的 。

+ **TreeMap**

  红黑树实现。能够把它保存的记录根据键(key)排序，默认是按升序排序，也可以指定排序的比较器，自定义类型做key，必须实现Comparable接口，实现compareTo方法。当用Iterator 遍历TreeMap时，得到的记录是排过序的。TreeMap不允许key的值为null。非同步的。
...
+ **Properties**
  ...
  Hashtable的子类，key和value都是String，通常用于配置文件的处理。

+ **ConcurrentHashMap**

  JDK7.0(包含)之前，采用分段锁机制，默认将HashMap分为16段，允许任意线程的读，一定数量的写。JDK8.0也引入红黑树实现。
...
static int indexFor(int hash, int length) {
    ...
    return hash & (length-1);//hashCode % length == hashCode & (length-1)
}
```
...
final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
    Node<K,V>[] tab;//代表数组 
    Node<K,V> p;//p代表数组链表的第一个node
    int n, i;//n代表数组长度，i代表node在数组的位置
    ...
    if ((p = tab[i = (n - 1) & hash]) == null)
        ...
        // 如果该节点是代表红黑树的节点，调用红黑树的插值方法，本文不展开说红黑树
            ...
            for (int binCount = 0; ; ++binCount) {
                ...
                //判断p.next是否等于要插入的值，如果在该链表中找到了"相等"的 key
                if (e.hash == hash &&
...
final Node<K,V>[] resize() {
    ...
    if (newThr == 0) { // 对应使用 new HashMap(int initialCapacity) 初始化后，第一次 put 的时候
...
![](image\Map\Java7 ConcurrentHashMap.png)
...
private Segment<K,V> ensureSegment(int k) {
    ...
    Segment<K,V> seg;
    if ((seg = (Segment<K,V>)UNSAFE.getObjectVolatile(ss, u)) == null) { // 检查一遍该槽是否被其他线程初始化了。
...
![](image\Map\Java8 ConcurrentHashMap.png)
...
### 1. 为什么不用二叉查找树代替而选择红黑树？为什么不一直使用红黑树？
...
![1294463-20181119224633228-1433434325](image\Map\红黑树.png)
