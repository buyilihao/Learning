...
SpringBoot是一个**快速整合第三方框架**，**简化XML配置完全使用注解化**，**内置Http服务器**，最终是以Java应用程序执行

SpringBoot原理

1. 快速整合第三方框架，原理：Maven的子父依赖关系，相当于需要整合的环境的Jar封装好依赖信息。
2. 无配置文件，注解化，原理：使用@EnableWebMvc代替web.xml加载SpringMVC容器，使用Java代码编写SpringMVC配置初始化。传统项目 web.xml --> class文件
3. 内置Http服务器。原理：Java语言创建tomcat容器，加载class文件。
